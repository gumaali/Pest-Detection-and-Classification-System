...
def net_evaluation_NEW(actual, predict, predict_proba=None):
    Max = preValidation(actual=actual, predict=predict)
    array, [TP, TN, FP, FN] = findConfusionMatrix(actual, predict, Max)
    Dice = (2 * TP) / ((2 * TP) + FP + FN) * 100
    Jaccard = TP / (TP + FP + FN) * 100  # IOU and Jac card are same metrics
    ...
    if predict_proba is not None:
        y_score = predict_proba.flatten()
    else:
        y_score = y_pred.copy()
    AP_class1 = average_precision_score(y_true, y_score)
    AP_class0 = average_precision_score(1 - y_true, 1 - y_score)
    mAP = (AP_class1 + AP_class0) / 2
    macro_f1 = f1_score(y_true, y_pred, average='macro')

    EVAL = [TP, TN, FP, FN, Dice, Jaccard, accuracy, psnr, mse, sensitivity, specificity, precision, fpr, fnr, npv, fdr,
            f1score, mcc, AP_class0, AP_class1, mAP, macro_f1]
    # EVAL = [TP, TN, FP, FN, Dice, Jaccard, accuracy, psnr, mse, sensitivity, specificity, precision, fpr, fnr, npv,
    #         fdr, f1score, mcc]
    return EVAL
