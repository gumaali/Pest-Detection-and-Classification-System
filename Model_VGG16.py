...
def Model_VGG16(Train_Data, Train_Tar, Test_Data, Test_Tar, EP=None, BS=None, HN=None):
    if BS is None:
        BS = 4
    if HN is None:
        HN = 4096
    ...
    model.fit(x=Train, y=Train_Tar, epochs=EP, batch_size=BS, steps_per_epoch=5)
    pred = model.predict(Test)
    avg = np.mean(pred)
    pred[pred >= avg] = 1
    pred[pred < avg] = 0
    pred = np.asarray(pred).astype('int')
    Eval = ClassificationEvaluation(Test_Tar, pred)
    return Eval, pred
